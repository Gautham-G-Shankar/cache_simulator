...
int main(int argc, char *argv[]) {
    ...
    float mainMemoryPenalty = (20.0f + (blockSize / 16.0));
    ...
    float AATVC = accessTimeVC + (1.0000000 - VCHitRate) * mainMemoryPenalty;
    float AAT = accessTimeL1 + static_cast<float>(swapRequestRate) * AATVC;
    ...
    std::cout << "  g. number of swaps:\t\t\t\t\t" << cacheVC.get_swap_count() << std::endl;
    std::cout << "  h. combined L1+VC miss rate:\t\t\t\t"  << combinedMissrateL1 << std::endl;
        ...
        while (std::getline(_file_, _line_)) {
            ...
            if (operation == "r") {
                ...
                if (hitL1) {
                    ...
                    if (cacheVC.get_dirty_hit()) {
                        cacheL2.dirtyWrite(_dirtyTag_);
                        Wb++;
                        }
    ...
    float AATL2 = accessTimeL2 + static_cast<float>(missRateL2) * mainMemoryPenalty;
    float AATVC = accessTimeVC + (1.0000000 - VCHitRate) * AATL2;
    ...
    std::cout << "  e. number of swap requests:\t\t\t\t" << totSwapReq << std::endl;
    std::cout << "  f. swap request rate:\t\t\t\t\t" << swapRequestRate << std::endl;
    std::cout << "  g. number of swaps:\t\t\t\t\t" << totSwaps << std::endl;
    std::cout << "  h. combined L1+VC miss rate:\t\t\t\t"  << combinedMissrateL1 << std::endl;
    std::cout << "  i. number writebacks from L1/VC:\t\t\t" << cacheL1.get_dirty_writes() << std::endl;
    std::cout << "  j. number of L2 reads:\t\t\t\t" << cacheL2.get_read_count() << std::endl;
    std::cout << "  k. number of L2 read misses:\t\t\t\t" << cacheL2.get_read_misses() << std::endl;
